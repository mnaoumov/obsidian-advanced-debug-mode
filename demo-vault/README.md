...
- **Otherwise**, download `advanced-debug-mode-demo-vault.zip` from the [plugin releases](https://github.com/mnaoumov/obsidian-advanced-debug-mode/releases), unzip it, and in Obsidian choose `Manage vaults...` → `Open folder as vault` → select the `advanced-debug-mode-demo-vault-<version>` folder it unzips into.
