...
(function () {
  ...
  function k(Z) {
    ...
    function ab(ah) {
      ...
      ag = Y[af];
      if (ag) return ag;
      else {if ('0' <= af && af <= '7') return parseInt(ah.substring(1), 8);
        else {if (af === 'u' || af === 'x') return parseInt(ah.substring(2), 16);
    ...
    function X(am) {
      ...
      for (var ar = ao ? 1 : 0, aj = aq.length; ar < aj; ++ar) {
        ...
        if (/\\[bdsw]/i.test(ah)) ak.push(ah);
          ...
          af.push([ag, al]);
          if (!(al < 65 || ag > 122)) {
            if (!(al < 65 || ag > 90)) af.push([Math.max(65, ag) | 32, Math.min(al, 90) | 32]);
            if (!(al < 97 || ag > 122)) af.push([Math.max(97, ag) & ~32, Math.min(al, 122) & ~32]);
          }
        }
      }
  ...
  function a(V) {
    ...
    else if (window.getComputedStyle) S = document.defaultView.getComputedStyle(V, null).getPropertyValue('white-space');
  ...
  function i(T) {
    ...
    if (Y) {
      if (T.cStyleComments) {
        if (Y > 1) W.push([j, /^#(?:##(?:[^#]|#(?!##))*(?:###|$)|.*)/, null, '#']);
        else W.push([j, /^#(?:(?:define|elif|else|endif|error|ifdef|include|ifndef|line|pragma|undef|warning)\b|[^\r\n]*)/, null, '#']);
    ...
    if (U.length) S.push([z, new RegExp('^(?:' + U.replace(/[\s,]+/g, '|') + ')\\b'), null]);
    W.push([F, /^\s+/, null, ' \r\n\t\xA0']);
    S.push(
  ...
  function Q(V, ag) {
    ...
    var S;
    if (V.currentStyle) S = V.currentStyle.whiteSpace;
    else if (window.getComputedStyle) S = ac.defaultView.getComputedStyle(V, null).getPropertyValue('white-space');
  ...
  function c(U, V) {
    for (var S = V.length; --S >= 0;) {
      ...
      else if (window.console) console.warn('cannot override language handler %s', T);
    }
  }
  function q(T, S) {
    if (!(T && t.hasOwnProperty(T))) T = /^\s*</.test(S) ? 'default-markup' : 'default-code';
    return t[T];
  }
  c(K, ['default-code']);
  c(
  ...
  function b(ad) {
    ...
    var T = [];
    for (var aa = 0; aa < ac.length; ++aa) for (var Z = 0, V = ac[aa].length; Z < V; ++Z) T.push(ac[aa][Z]);
    ac = null;
    var W = Date;
    if (!W.now) {
      W = {
    ...
    function U() {
      ...
      for (; X < T.length && W.now() < ag; X++) {
        ...
        if (ai.indexOf('prettyprint') >= 0) {
          var ah = ai.match(ab);
          var am;
          if (!ah && (am = o(aj)) && 'CODE' === am.tagName) ah = am.className.match(ab);
          if (ah) ah = ah[1];
          var al = false;
